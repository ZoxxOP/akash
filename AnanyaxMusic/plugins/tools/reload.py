...
from AnanyaxMusic.utils.database import get_assistant, get_authuser_names, get_cmode
from AnanyaxMusic.utils.decorators import ActualAdminCB, AdminActual, language
from AnanyaxMusic.utils.formatters import alpha_to_int, get_readable_time
...
@akash.on_message(filters.command(["reboot"]) & filters.group & ~BANNED_USERS)
@AdminActual
async def restartbot(client, message: Message, _):
    mystic = await message.reply_text(_["reload_4"].format(akash.mention))
    ...
    if chat_id:
        try:
            got = await akash.get_chat(chat_id)
        except:
            pass
        ...
        try:
            db[chat_id] = []
            await Ananya.stop_stream_force(chat_id)
        except:
            pass
    return await mystic.edit_text(_["reload_5"].format(akash.mention))


@akash.on_callback_query(filters.regex("close") & ~BANNED_USERS)
...
@akash.on_callback_query(filters.regex("stop_downloading") & ~BANNED_USERS)
@ActualAdminCB
