...
from AnanyaxMusic.utils import AnanyaBin, get_channeplayCB, seconds_to_min
from AnanyaxMusic.utils.database import get_cmode, is_active_chat, is_music_playing
from AnanyaxMusic.utils.decorators.language import language, languageCB
from AnanyaxMusic.utils.inline import queue_back_markup, queue_markup
...
async def get_queue(client, message: Message, _):
    ...
    cap = _["queue_8"].format(akash.mention, title, typo, user, send)
    upl = (
...
@akash.on_callback_query(filters.regex("GetTimer") & ~BANNED_USERS)
...
@akash.on_callback_query(filters.regex("GetQueued") & ~BANNED_USERS)
@languageCB
...
@akash.on_callback_query(filters.regex("queue_back_timer") & ~BANNED_USERS)
@languageCB
